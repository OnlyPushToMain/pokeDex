...
import { colorChange, colorChangeBG, colorChangeBG2, colorChangeStats } from "./ColorFunctions.js"

export default function Card ({ pokemon, loading }) {
  return (
    <div className="wrapper">
      {loading && <div>
        <img className={pokemon[0].types.length > 1 ? `bg-gradient-to-r ${colorChangeBG(pokemon[0].types[0].type.name)} ${colorChangeBG2(pokemon[0].types[1].type.name)} w-full object-cover object-center rounded shadow-md` : `${colorChange(pokemon[0].types[0].type.name)} w-full object-cover object-center rounded shadow-md`} src={pokemon[0].sprites.other["official-artwork"].front_default} alt=""/>
        <div>
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="flex flex-row capitalize">
                  {pokemon[0].types.map((value, index) => {
                    return (
                      <div key={index}
                        className={`${colorChange(value.type.name)} brightness-125 rounded shadow-md ml-1 mr-1`}>
