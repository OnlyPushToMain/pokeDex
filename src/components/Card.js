...
export default function Card ({ pokemon2, loading2, colors, colorsBG, colorsBG2 }) {
  return (
    <div className="wrapper">
      {loading2 && <div>
        <img className={pokemon2[0].types.length === 2 ? `bg-gradient-to-r  ${colorsBG[`${pokemon2[0].types[0].type.name}`]} ${colorsBG2[`${pokemon2[0].types[1].type.name}`]} w-full object-cover object-center rounded shadow-md` : `${colors[`${pokemon2[0].types[0].type.name}`]} w-full object-cover object-center rounded shadow-md`} src={pokemon2[0].sprites.other["official-artwork"].front_default} alt=""/>
        <div>
